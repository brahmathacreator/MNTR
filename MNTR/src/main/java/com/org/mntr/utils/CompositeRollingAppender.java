...
public class CompositeRollingAppender extends FileAppender {
	...
	public CompositeRollingAppender(Layout layout, String s, String s1) throws IOException {
		this(layout, s, s1, true);
	}

	public CompositeRollingAppender(Layout layout, String s, boolean flag) throws IOException {
		super(layout, s, flag);
		datePattern = "'.'yyyy-MM-dd";
		dateExpireInterval = -1L;
		scheduledFilename = null;
	...
	public CompositeRollingAppender(Layout layout, String s, String s1, boolean flag) throws IOException {
		super(layout, s, flag);
		datePattern = "'.'yyyy-MM-dd";
		dateExpireInterval = -1L;
		scheduledFilename = null;
	...
	protected void subAppend(LoggingEvent loggingevent) {
		...
		if (rollSize && fileName != null && ((CountingQuietWriter) qw).getCount() >= maxFileSize)
			rollOverSize();
	...
	protected void existingInit() {
		...
		if (afile != null) {
			LogLog.debug("existingInit: Found " + afile.length + " file in directory: " + file1.getAbsolutePath());
			for (int i = 0; i < afile.length; i++) {
				File file3 = afile[i];
				String s1 = file3.getAbsolutePath();
				LogLog.debug("existingInit: files[" + i + "] = " + s1 + " baseFileName = " + s);
				if (!s1.startsWith(s))
					continue;
				int j = s1.lastIndexOf(".");
				if (staticLogFileName) {
					if (j != s.length()) {
						LogLog.debug("existingInit: Skiping from backup count: " + file3);
						continue;
					}
					LogLog.debug("existingInit: Found in backup count: " + file3);
				}
				try {
					int k = Integer.parseInt(s1.substring(j + 1, s1.length()));
					if (k > curSizeRollBackups) {
						LogLog.debug("From file: " + file3 + " -> curSizeRollBackups set to: " + k);
						curSizeRollBackups = k;
					}
				} catch (Exception exception) {
					LogLog.debug("Encountered a backup file not ending in .x " + file3);
				}
			}
	...
	public void rollOverTime() {
		...
		if (-1L < dateExpireInterval && staticLogFileName) {
			...
			if (scheduledFilename.equals(fileName + s)) {
				errorHandler.error("Compare " + scheduledFilename + " : " + fileName + s);
				return;
			}
			closeFile();
